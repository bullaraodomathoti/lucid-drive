...
\caption{Multi-modal sensor fusion bird's-eye-view (BEV) rendered from real
  nuScenes data (scene \texttt{n015-2018-07-24-11-22-45+0800}, urban driving,
  Singapore).
  \textbf{LiDAR}: 33{,}880 height-coloured points (plasma colourmap, colourbar
  at right) from a Velodyne HDL-32E sweep spanning $\pm50$\,m.
  \textbf{Camera FOVs}: six colour-coded wedges ($70^{\circ}$ each) show the
  surround-view camera coverage; labels denote mounting positions
  (FRONT, FRONT\_LEFT, FRONT\_RIGHT, BACK, BACK\_LEFT, BACK\_RIGHT).
  \textbf{3D annotations}: 69 oriented bounding-box footprints (coloured by
  class: pedestrian, car, truck, bus, barrier, traffic cone, \emph{etc.})
  projected into the BEV plane from ground-truth 3D annotations.
  \textbf{Radar}: 17 cyan diamond markers indicate instances with associated
  radar returns; velocity arrows are scaled to a $1.5$\,s lookahead.
  \textbf{Yellow}: ego vehicle with forward-heading arrow.
  All modalities are aligned in the ego-vehicle reference frame.}
